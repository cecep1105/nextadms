...
function BreadcrumbSegmentView({ segment, isLast }: { segment: BreadcrumbSegment; isLast: boolean }) {
  const textClass = isLast ? "font-medium text-foreground" : "text-muted-foreground";
  ...
  if (segment.options.length === 0) {
    ...
    return segment.href ? (
      ...
      <span className={`truncate ${textClass}`}>{segment.label}</span>
    );
  }
    ...
    <div className="flex min-w-0 flex-1 items-center gap-1.5 text-sm">
      {segments.map((segment, i) => (
        <span key={`${segment.label}-${i}`} className="flex min-w-0 items-center gap-1.5">
          ...
          <BreadcrumbSegmentView segment={segment} isLast={i === segments.length - 1} />
        </span>
      ))}
    </div>
  );
