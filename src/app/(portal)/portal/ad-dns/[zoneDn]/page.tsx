...
// VIEW-ONLY (SENGAJA, beda dari halaman staff) -- tambah/edit/hapus
// record (AddRecordButton/RecordActionsMenu) SENGAJA dikecualikan dari
// cakupan izin portal can_view_ad_dns (lihat netmgmt/active_directory_dns_view.py::
// ADDNSRecordActionView, permission-nya TETAP staff-only).
...
export default async function PortalAdDnsRecordsPage({
  params, searchParams,
}: {
  params: { zoneDn: string };
  searchParams: { sortBy?: string; sortDir?: string; page?: string; q?: string; page_size?: string };
}) {
  const zoneDn = decodeURIComponent(params.zoneDn);
  const pageSize = Number(searchParams.page_size ?? PAGE_SIZE);
  const data = await getRecords(zoneDn, searchParams.sortBy, searchParams.sortDir, searchParams.page, searchParams.q, searchParams.page_size);

  return (
    <div>
      <PageHeader
        title="Active Directory - DNS Records"
        description={
          <div className="flex flex-col gap-1">
            <Link href="/portal/ad-dns" className="inline-flex items-center gap-1 text-primary hover:underline">
        ...
        <Table>
          ...
          <TableBody>
            {data.results.length === 0 ? (
              <TableRow><TableCell colSpan={4} className="py-8 text-center text-muted-foreground">Tidak ada record ditemukan.</TableCell></TableRow>
            ) : (
