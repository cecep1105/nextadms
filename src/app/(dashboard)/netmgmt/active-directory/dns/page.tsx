...
export default async function ActiveDirectoryDnsZonesPage() {
  const data = await apiServerFetch<{ count: number; results: DnsZone[]; partition_errors: string[] }>("/netmgmt/ad/dns/zones/");

  return (
    <div>
      <PageHeader
        title="NetMgmt / Active Directory / DNS Zones"
        description="Zone DNS AD-integrated -- dicari di ketiga kemungkinan partisi (Forest/Domain/Legacy) sekaligus."
      />
      <Card>
        <Table>
          <TableHeader>
                  ...
                  <TableRow key={zone.dn}>
                    ...
                    <TableCell>
                      <div className="flex justify-end">
                        <Link
                          href={`/netmgmt/active-directory/dns/${encodeURIComponent(zone.dn)}`}
