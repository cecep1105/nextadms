 ...
 * sudah ada (BUKAN endpoint baru), persis pola yang dipakai
 * netwatch-actions-menu.tsx utk enable/disable (`?postcmd=add`/`set`,
 * body diteruskan APA ADANYA ke RouterOS `.call()`) -- sudah diverifikasi
 * bekerja lewat test langsung sebelum dibangun.
 */
export function NetwatchFormDialog({
  mode, basePath, item, open, onOpenChange,
}: {
  mode: "add" | "edit";
  basePath: string;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await request(`${basePath}/?postcmd=${mode === "add" ? "add" : "set"}`, {
        method: "POST",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Up-script/down-script dijalankan RouterOS sendiri saat status host berubah -- lihat contoh di test/netwatchscript.txt.
          </DialogDescription>
        </DialogHeader>
