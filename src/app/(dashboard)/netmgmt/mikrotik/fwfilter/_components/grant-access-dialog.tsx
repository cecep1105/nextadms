 ...
 * penyalinan/penyisipan rule SEPENUHNYA di backend (lihat
 * netmgmt/routeros_firewall_view.py::FirewallGrantAccessView) -- form ini
 * cuma kumpulkan input & tampilkan hasil lookup DHCP.
 */
export function GrantAccessDialog({ routerHost }: { routerHost: string }) {
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    try {
      const data = await request<{ results: MikrotikDhcpLease[] }>(
        `/netmgmt/routeros/${routerHost}/ip-dhcp_server-lease/?_q=${encodeURIComponent(ipAddress)}&_search_fields=address&_limit=1`
      );
